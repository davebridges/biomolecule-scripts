...
<<data-input>>=
setwd("") #set working directory for file
data_file <- "somefile.csv" #set input file name
raw_data <- read.csv(data_file, skip=10, header=T) #skip first 10 lines of output file
...
<<label=summary-tables,echo=FALSE,results=tex>>=
print(xtable(means, caption = "Averaged Expression Values", label = "tab:one", include.rownames=F))
@
